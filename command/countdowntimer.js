const countdownTimerSchema = require('../schema/countdowntimer');
const countdownTimerConfigSchema = require('../schema/countdowntimerconfig');
...
async function countdownTimer(channelID, argument) {
    const cacheClient = getClient();

    let countdownTimer = await cache.get(`${channelID}:countdown:timer`);
    let countdownTimerConfig = await cache.get(`${channelID}:countdown:config`);

    if(!countdownTimer) {
        countdownTimer = await countdownTimerSchema.findOne({channelID: channelID});
        if(!countdownTimer) {
            return {
                error: true,
                message: 'No countdown timer has been set for this channel',
                status: 400,
                type: 'error'
            }
        }
    ...
    if(!countdownTimerConfig) {
        countdownTimerConfig = await countdownTimerConfigSchema.findOne({channelID: channelID});
        if(!countdownTimerConfig) {
            return {
                error: true,
                message: 'No countdown timer config has been set for this channel',
                status: 400,
                type: 'error'
            }
        }
    ...
    switch(argument[0]) {
        case 'start':
            let startResponse = await fetch(`${getUrl()}/countdowntimer/${channelID}/timer/resume`, {
                method: 'POST',
            })

            let startData = await response.json();

            if (startData.error) return {error: true, message: startData.message};
            return {error: false, message: startData.message};
            break;
        case 'pause':
            let pauseResponse = await fetch(`${getUrl()}/countdowntimer/${channelID}/timer/resume`, {
                method: 'POST',
            })

            let pauseData = await response.json();

            if (pauseData.error) return {error: true, message: pasueData.message};
            return {error: false, message: pasueData.message};
            break;
        case 'resume':
            let resumeResponse = await fetch(`${getUrl()}/countdowntimer/${channelID}/timer/resume`, {
                method: 'POST',
            })

            let resumeData = await response.json();

            if (resumeData.error) return {error: true, message: resumeData.message};
            return {error: false, message: resumeData.message};
            break;
        case 'add':
            donationTime = parseInt(argument[1]);
            break;
